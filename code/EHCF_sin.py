...
def parse_args():
    parser = argparse.ArgumentParser(description="Run EHCH")
    parser.add_argument('--dataset', nargs='?', default='ml-1m',
                        help='Choose a dataset: ml-1m,epinion,ciao')
...
if __name__ == '__main__':
    ...
    if args.dataset == 'ml-1m':
        ...
        f1 = open(os.path.join(DATA_ROOT, 'EHCF_sin.txt'), 'w')
    ...
    if args.dataset == 'epinion':
        print('load epinion data')
        DATA_ROOT = '../data/epinion'
        f1 = open(os.path.join(DATA_ROOT, 'EHCF_sin.txt'), 'w')

        tp_test = load_data(os.path.join(DATA_ROOT, 'epinion.test.txt'))
        tp_train = load_data(os.path.join(DATA_ROOT, 'epinion.train.txt'))
    if args.dataset == 'ciao':
        print('load ciao data')
        DATA_ROOT = '../data/ciao'
        f1 = open(os.path.join(DATA_ROOT, 'EHCF_sin.txt'), 'w')

        tp_test = load_data(os.path.join(DATA_ROOT, 'ciao.test.txt'))
        tp_train = load_data(os.path.join(DATA_ROOT, 'ciao.train.txt'))
    if args.dataset == 'Beibei':
        print('load beibei data')
    ...
    item_lenth=[]
    for i in train_set:
        item_lenth.append(len(train_set[i]))
    item_lenth.sort()
    print item_lenth
    max_item_pu = item_lenth[int(len(item_lenth)*0.999)]
    print max_item_pu
    for i in train_set:
        if len(train_set[i]) > max_item_pu:
            train_set[i]=train_set[i][0:max_item_pu]
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            deep = EHCF(n_users, n_items, embedding_size, max_item_pu,args)
            deep._build_graph()
            ...
            for epoch in range(epochs):
                ...
                if epoch < epochs:
                    if epoch % args.verbose == 0:
                        dev_cold(u_train, i_train, test_set, train_m, test_m)
